...
void M5dramSystem::moveToTime(const tick now)
{
	...
	while (finishedTransactions.size() > 0)
	{
		...
		std::tr1::unordered_map<unsigned, Packet*>::iterator packetIterator =
				transactionLookupTable.find(currentValue.first);
		...
		if (packet)
		{
			...
			if (needsResponse)
			{
				...
				assert(curTick < static_cast<Tick> (currentValue.second
						* getCPURatio()));

				DPRINTF(
						MemoryAccess,
						"<-T [@%d] [+%d]",
						static_cast<Tick> (currentValue.second * getCPURatio()),
						static_cast<Tick> (currentValue.second * getCPURatio()
								- curTick));

				ports[lastPortIndex]->doSendTiming(packet,
						static_cast<Tick> (currentValue.second * getCPURatio()));
				...
				if (++returnCount % 100000 == 0)
				{
					...
					//*(pos-1) = NULL;
					cerr << "\r" << std::dec << returnCount / 100000 << " "
							<< asctime(timeinfo) << "\r";
				}
#endif
			}
			else
			{
				...
				DPRINTF(MemoryAccess, "xT [%x]", packet->getAddr());
		...
		else
		{
			...
			cerr << "warn: no Packet found for corresponding transaction"
					<< endl;
#endif
		}
	}
...
M5dramSystem::M5dramSystem(const Params *p) :
	PhysicalMemory(p), tickEvent(this), ports(1), lastPortIndex(0), ds(NULL),
			cpuRatio(0), transactionLookupTable(), currentTransactionID(0),
			movement(true), outstandingPackets(0)
{
	Settings settings;
	...
	settings.setKeyValue("addressMappingScheme",
			p->physicalAddressMappingPolicy);
	...
	settings.setKeyValue("commandOrderingAlgorithm",
			p->commandOrderingAlgorithm);
	settings.setKeyValue("transactionOrderingAlgorithm",
			p->transactionOrderingAlgorithm);
	...
	cpuRatio = (int) round(((float) SimClock::Frequency
			/ ((float) ds->Frequency())));

	DPRINTF(MemoryAccess, "%s", *ds);
}
...
Port *M5dramSystem::getPort(const string &if_name, int idx)
{
	...
	lastPortIndex = idx;
	ports[idx] = port;
	DPRINTF(MemoryAccess, "called M5dramSystem::getPort");
	return port;
}
...
{
	DPRINTF(MemoryAccess, "M5dramSystem destructor");
...
void M5dramSystem::unserialize(Checkpoint *cp, const std::string &section)
{
	...
	pmemAddr = (uint8_t *) mmap(NULL, params()->range.size(), PROT_READ
	...
	while (curSize < params()->range.size())
	{
		...
		if (bytesRead != chunkSize && bytesRead != params()->range.size()
				- curSize)
			fatal("Read failed on physical memory checkpoint file '%s'"
				" got %d bytes, expected %d or %d bytes\n", filename,
					bytesRead, chunkSize, params()->range.size() - curSize);
...
bool M5dramSystem::MemoryPort::recvTiming(PacketPtr packet)
{
	...
	using std::setw;
	stringstream ss;
	ss << "+recvTiming [" << std::dec << curTick << "] ";
	// calculate the time elapsed from when the transaction started
	ss << setw(2) << (packet->isRead() ? "Rd" : "");
	ss << setw(2) << (packet->isWrite() ? "Wr" : "");
	//M5_TIMING2((packet->isRequest() ? "Rq" : "");
	ss << setw(3) << (packet->isInvalidate() ? "Inv" : "");
	ss << setw(3) << (packet->isResponse() ? "Rsp" : "");
	ss << setw(2) << (packet->isReadWrite() ? "RW" : "");
	ss << setw(2) << (packet->isError() ? "Er" : "");
	ss << setw(2) << (packet->isPrint() ? "Pr" : "");
	ss << setw(2) << (packet->needsExclusive() ? "Ex" : "");
	ss << setw(2) << (packet->needsResponse() ? "NR" : "");
	ss << setw(2) << (packet->isLLSC() ? "LL" : "");
	ss << " 0x" << hex << packet->getAddr() << endl;
	ss << " s[0x" << hex << packet->getSize() << "]";
	...
	if (packet->isRead() || packet->isWrite())
	{
		...
		if (memory->ds->isFull(addr.getChannel()))
		{
			...
			DPRINTF(MemoryAccess,
					"Wait for retry before sending more to ch[%d]",
					addr.getChannel());
			return false;
		}
		else
		{
			tick currentMemCycle = (curTick + memory->getCPURatio() - 1)
					/ memory->getCPURatio();
			...
			int threadID =
					packet->req->hasContextId() ? packet->req->contextId() : 0;

			PhysicalAddress pC = packet->req->hasPC() ? packet->req->getPC()
					: 0;

			Transaction::TransactionType
					packetType =
							packet->req->isPrefetch() ? Transaction::PREFETCH_TRANSACTION
			...
			assert(packet->isRead() ^ (Transaction::WRITE_TRANSACTION
					== packetType));
			assert(packet->isWrite() ^ (Transaction::READ_TRANSACTION
					== packetType));

#ifndef NDEBUG
			bool result =
#endif
					memory->ds->enqueue(new Transaction(packetType,
							currentMemCycle, packet->getSize() / 8,
							packet->getAddr(), pC, threadID,
							memory->currentTransactionID));

			(memory->outstandingPackets)++;

			assert(result == true);

			assert(memory->transactionLookupTable.find(
					memory->currentTransactionID)
					== memory->transactionLookupTable.end());
			...
			delete packet;
#else
			memory->transactionLookupTable[memory->currentTransactionID]
					= packet;
#endif

			// make sure not to use any that are already being used
			while (memory->transactionLookupTable.find(
					memory->currentTransactionID)
					!= memory->transactionLookupTable.end())
			{
				memory->currentTransactionID = (memory->currentTransactionID
						+ 1) % UINT_MAX;
			}
			...
			DPRINTF(MemoryAccess, "-recvTiming sch[%d]", next);
		}
	}
	else
	{
...
void M5dramSystem::TickEvent::process()
{
	...
	DPRINTF(MemoryAccess, "+process [%d]", currentMemCycle);
	...
	DPRINTF(MemoryAccess, "-process sch[%d]", next);
}
