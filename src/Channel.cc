...
/// @author Joe Gross
//////////////////////////////////////////////////////////////////////////
Channel::Channel(const Settings& _settings,
		const SystemConfiguration& _systemConfig, Statistics& _stats) :
	time(0ll), lastCommandIssueTime(-1ll * _settings.tCMD), lastCommand(NULL),
			timingSpecification(_settings), transactionQueue(
					_settings.transactionQueueDepth), refreshCounter(
					_settings.rankCount), systemConfig(_systemConfig),
			statistics(_stats), powerModel(_settings), channelID(UINT_MAX),
			rank(_settings.rankCount * _settings.dimmCount, Rank(_settings,
					timingSpecification, _systemConfig, _stats)),
			finishedTransactions(), lastCprhLocation(0)
{
	setupCprhValues();
#if 0
	...
	if (_settings.refreshPolicy != NO_REFRESH)
	{
		...
		for (unsigned j = 0; j < refreshCounter.size(); ++j)
		{
			...
			refreshCounter[j] = new Transaction(
					Transaction::AUTO_REFRESH_TRANSACTION, j * step + 1, 8,
					addr, 0, 0);
...
/// @author Joe Gross
//////////////////////////////////////////////////////////////////////////
Channel::Channel(const Channel& rhs, const SystemConfiguration& systemConfig,
		Statistics& stats) :
			time(rhs.time),
			lastCommandIssueTime(rhs.lastCommandIssueTime),
			lastCommand(rhs.lastCommand ? new Command(*rhs.lastCommand) : NULL),
			timingSpecification(rhs.timingSpecification), transactionQueue(
					rhs.transactionQueue), refreshCounter(
					rhs.refreshCounter.size()), systemConfig(systemConfig),
			statistics(stats), powerModel(rhs.powerModel),
			channelID(rhs.channelID),
			// to initialize the references
			rank((unsigned) systemConfig.getRankCount()
					* systemConfig.getDimmCount(), Rank(rhs.rank[0],
					timingSpecification, systemConfig, stats)),
			finishedTransactions(), cprhSequence(rhs.cprhSequence),
...
/// @author Joe Gross
//////////////////////////////////////////////////////////////////////////
Channel::Channel(const Settings& settings, const SystemConfiguration& sysConf,
		Statistics &stats, const PowerConfig &power,
		const std::vector<Rank> &newRank, const TimingSpecification &timing) :
	time(0), lastCommandIssueTime(-1ll * settings.tCMD), lastCommand(NULL),
			timingSpecification(timing), transactionQueue(0),
			refreshCounter(0), systemConfig(sysConf), statistics(stats),
			powerModel(power), channelID(UINT_MAX), rank(newRank),
			finishedTransactions(), lastCprhLocation(0)
{
}
...
Channel::Channel(const Channel &rhs) :
	time(rhs.time), lastCommandIssueTime(rhs.lastCommandIssueTime),
			lastCommand(rhs.lastCommand), timingSpecification(
					rhs.timingSpecification), transactionQueue(
					rhs.transactionQueue), refreshCounter(
					rhs.refreshCounter.size()), systemConfig(rhs.systemConfig),
			statistics(rhs.statistics), powerModel(rhs.powerModel), channelID(
					rhs.channelID),
			rank((unsigned) rhs.rank.size(), Rank(rhs.rank[0],
					timingSpecification, systemConfig, statistics)),
			finishedTransactions(), cprhSequence(rhs.cprhSequence),
			lastCprhLocation(rhs.lastCprhLocation)
{
...
#if 1
	for (vector<Transaction *>::iterator i = refreshCounter.begin(), end =
			refreshCounter.end(); i != end; ++i)
	{
		delete *i;
		*i = NULL;
	}
...
void Channel::moveToTime(const tick currentTime)
{
	...
	while (time < currentTime)
	{
		...
		time = max(min(currentTime, min(nextTransactionDecodeTime(),
				nextCommandExecuteTime())), time);
		...
		while (Transaction *nextTransaction = getTransaction())
		{
			...
			DPRINTF(
					...
					transactionQueue.depth(),
					rank[nextTransaction->getAddress().getRank()].bank[nextTransaction->getAddress().getBank()].size(),
					rank[nextTransaction->getAddress().getRank()].bank[nextTransaction->getAddress().getBank()].depth(),
					*nextTransaction);
...
{
	return max(min(min(nextTransactionDecodeTime(), nextCommandExecuteTime()),
	...
	if (const Transaction *nextTrans = readTransaction(false))
	{
		if (nextTrans)
		{
			if (nextTrans->isRefresh())
				...
				return nextTrans->getEnqueueTime()
						+ timingSpecification.tBufferDelay();
		}
	}
...
//////////////////////////////////////////////////////////////////////////
void Channel::getPendingTransactions(
...
tick Channel::nextCommandExecuteTime() const
{
	...
	if (const Command *tempCommand = readNextCommand())
	{
		...
		if (time + tempGap != tempCommandExecuteTime)
			cerr << time << " " << tempGap << " " << tempCommandExecuteTime
					<< " " << tempCommand->getCommandType() << " "
					<< tempCommand->getAddress() << endl;
		;
...
void Channel::retireCommand(Command *newestCommand, const bool isHit)
{
	...
	assert(((newestCommand->isReadOrWrite() || newestCommand->isRefresh())
			&& newestCommand->getHost()) || (!(newestCommand->isReadOrWrite()
			|| newestCommand->isRefresh()) && !newestCommand->getHost()));
	...
	{
		DPRINTF(MemoryAccess, "-T %s", *completedTransaction);

		if (!newestCommand->isRefresh())
		{
			statistics.collectTransactionStats(completedTransaction);

			assert(newestCommand->getEnqueueTime()
					>= completedTransaction->getEnqueueTime()
					&& newestCommand->getCompletionTime()
							<= completedTransaction->getCompletionTime());

			const unsigned origTrans =
					completedTransaction->getOriginalTransaction();

			M5_DEBUG(assert(origTrans < UINT_MAX));

			if (origTrans < UINT_MAX)
				finishedTransactions.push(std::pair<unsigned, tick>(origTrans,
						completedTransaction->getCompletionTime()));
		}
		else
		{
...
bool Channel::enqueue(Transaction *incomingTransaction)
{
	...
	case RIFF:
		...
		{
			for (int currentIndex = transactionQueue.size() - 1; currentIndex
					>= 0; --currentIndex)
			{
				// prevent against starvation and RAW errors, reads do not switch their ordering
				if (transactionQueue[currentIndex]->isRead() || (time
						- transactionQueue[currentIndex]->getEnqueueTime()
						> systemConfig.getSeniorityAgeLimit())
						|| transactionQueue[currentIndex]->getAddress()
								== incomingTransaction->getAddress())
				{
					result = transactionQueue.insert(incomingTransaction,
							currentIndex + 1);
					assert(result);
					return result;
				}
...
Transaction::TransactionType Channel::setReadWriteType(const int rankID) const
{
	...
	DEBUG_LOG("Rank[" << rankID << "] Read[" << readCount << "] Write["
			<< writeCount << "] Empty[" << emptyCount << "]");
...
ostream &Channel::doPowerCalculation(ostream& os)
{
	...
	for (vector<Rank>::iterator k = rank.begin(); k != rank.end(); ++k)
	{
		...
		double percentActive = 1.0 - (k->getPrechargeTime(time) / max(
				(double) (time - powerModel.getLastCalculation()), 0.00000001));

		assert(percentActive >= 0.0F && percentActive <= 1.0F);
		assert(k->getPrechargeTime(time) <= time
				- powerModel.getLastCalculation());
		...
		double PschACT_STBY = powerModel.getPdsACT_STBY() * percentActive * (1
				- CKE_LO_ACT);
		PsysACT_STBY += powerModel.getDevicesPerRank()
				* powerModel.getVoltageScaleFactor()
				* powerModel.getFrequencyScaleFactor() * PschACT_STBY;

		// calculate PsysPRE-STBY
		double PschPRE_STBY = powerModel.getPdsPRE_STBY() * (1.0
				- percentActive) * (1 - CKE_LO_PRE);
		PsysPRE_STBY += powerModel.getDevicesPerRank()
				* powerModel.getFrequencyScaleFactor()
				* powerModel.getVoltageScaleFactor() * PschPRE_STBY;

		// calculate PsysPRE-PDN
		double PschPRE_PDN = powerModel.getPdsPRE_PDN() * (1.0 - percentActive)
				* (CKE_LO_PRE);
		PsysPRE_PDN += powerModel.getDevicesPerRank()
				* powerModel.getFrequencyScaleFactor()
				* powerModel.getVoltageScaleFactor() * PschPRE_PDN;

		// calculate PsysACT-PDN
		double PschACT_PDN = powerModel.getPdsACT_PDN() * percentActive
				* CKE_LO_ACT;
		PsysACT_PDN += powerModel.getDevicesPerRank()
				* powerModel.getFrequencyScaleFactor()
				* powerModel.getVoltageScaleFactor() * PschACT_PDN;

		// calculate PsysACT
		double tRRDsch = ((double) (time - powerModel.getLastCalculation()))
				/ (thisRankRasCount > 0 ? thisRankRasCount : 0.00000001);
...
#endif
		PsysACT += powerModel.getDevicesPerRank()
				* powerModel.getVoltageScaleFactor() * PschACT;

		//PsysACTArray.push_back(PsysACT);

		double RDschPct = k->getReadCycles() / (double) (time
				- powerModel.getLastCalculation());

		PsysRD += powerModel.getDevicesPerRank()
				* powerModel.getVoltageScaleFactor()
				* powerModel.getFrequencyScaleFactor() * powerModel.getPdsRD()
				* RDschPct;

		double WRschPct = k->getWriteCycles() / (double) (time
				- powerModel.getLastCalculation());

		PsysWR += powerModel.getDevicesPerRank()
				* powerModel.getVoltageScaleFactor()
				* powerModel.getFrequencyScaleFactor() * powerModel.getPdsWR()
				* WRschPct;

		os << " rk[" << k->getRankId() << "] prechargeTime{"
				<< k->getPrechargeTime(time) << "} rasCount{"
				<< thisRankRasCount << "} duration{" << time
				- powerModel.getLastCalculation() << "} read{"
				<< k->getReadCycles() << "} readHits{" << k->getReadHits()
	...
	os << "-Psys(ACT_STBY) ch[" << channelID << "] {" << setprecision(5)
			<< PsysACT_STBY << "} mW " << endl;

	os << "-Psys(ACT) ch[" << channelID << "] {" << setprecision(5) << PsysACT
			<< "} mW" << endl;

	os << "-Psys(PRE_STBY) ch[" << channelID << "] {" << setprecision(5)
			<< PsysPRE_STBY << "} mW" << endl;

	os << "-Psys(RD) ch[" << channelID << "] {" << setprecision(5) << PsysRD
			<< "} mW" << endl;

	os << "-Psys(WR) ch[" << channelID << "] {" << setprecision(5) << PsysWR
			<< "} mW" << endl;
...
const Transaction *Channel::readTransaction(const bool bufferDelay) const
{
	...
	const Transaction *nextTransaction =
			(val < UINT_MAX) ? transactionQueue[val] : NULL;

	if (systemConfig.getRefreshPolicy() == NO_REFRESH)
	{
		if ((nextTransaction) && (time >= nextTransaction->getEnqueueTime()
				+ delay))
		{
	...
	else
	{
		...
		if (nextTransaction && ((time < nextRefresh
				+ systemConfig.getSeniorityAgeLimit()) && (time
				>= nextTransaction->getEnqueueTime() + delay)))
		{
			return nextTransaction;
		}
		// either there is no transaction or the refresh command will starve
		else if (((time >= nextRefresh) || !bufferDelay)
				&& (checkForAvailableCommandSlots(readNextRefresh())))
		{
...
unsigned Channel::readAvailableTransaction(const bool bufferDelay) const
{
	...
	unsigned limit = min(systemConfig.getDecodeWindow(),
	...
	for (unsigned i = 0; i < limit; ++i)
	{
		...
		if ((enqueueTime + delay <= time) && checkForAvailableCommandSlots(
				currentTrans))
		{
			bool conflict = false;
			...
			{
				if (currentTrans->getAddress()
						== transactionQueue[j]->getAddress())
				{
					conflict = true;
					break;
				}
...
Transaction *Channel::getTransaction()
{
	...
	else
	{
		...
		if (nextTransaction && ((time < nextRefresh
				+ systemConfig.getSeniorityAgeLimit()) && (time
				>= nextTransaction->getEnqueueTime()
						+ timingSpecification.tBufferDelay())))
		{
			return getAvailableTransaction(val);
		}
		else if ((time >= nextRefresh) && (checkForAvailableCommandSlots(
				readNextRefresh())))
		{
...
{
	Transaction *newRefreshTransaction =
			refreshCounter[readNextRefresh()->getAddress().getRank()];
	assert(refreshCounter[newRefreshTransaction->getAddress().getRank()]
			== newRefreshTransaction);

	unsigned rank = newRefreshTransaction->getAddress().getRank();

	refreshCounter[rank] = new Transaction(
			Transaction::AUTO_REFRESH_TRANSACTION,
			newRefreshTransaction->getArrivalTime()
					+ timingSpecification.tREFI(), 8,
			newRefreshTransaction->getAddress(), 0, 0);
...
const Transaction *Channel::readNextRefresh() const
{
	...
	for (vector<Transaction *>::const_iterator end = refreshCounter.end(); i
			!= end; ++i)
	{
...
void Channel::resetToTime(const tick time)
{
	...
	for (vector<Transaction *>::iterator i = refreshCounter.begin(); i
			!= refreshCounter.end(); ++i)
	{
...
//////////////////////////////////////////////////////////////////////
bool Channel::checkForAvailableCommandSlots(
		const Transaction *incomingTransaction) const
{
	...
	assert(incomingTransaction->getAddress().getChannel() == channelID
			|| incomingTransaction->isRefresh());

	/// @todo switch to iterator arithmetic
	const Bank
			&destinationBank =
					rank[incomingTransaction->getAddress().getRank()].bank[incomingTransaction->getAddress().getBank()];

	unsigned
			availableCommandSlots =
					(incomingTransaction->isRefresh()) ? 0
							: rank[incomingTransaction->getAddress().getRank()].bank[incomingTransaction->getAddress().getBank()].freeCommandSlots();
	...
	case CLOSE_PAGE:
		...
		if (incomingTransaction->isRefresh())
		{
			const Rank &destinationRank =
					rank[incomingTransaction->getAddress().getRank()];
			...
			// are available
			for (vector<Bank>::const_iterator currentBank =
					destinationRank.bank.begin(); currentBank
					!= destinationRank.bank.end(); ++currentBank)
			{
		...
		else if (systemConfig.getRowBufferManagementPolicy()
				== CLOSE_PAGE_AGGRESSIVE
				&& destinationBank.closePageAggressiveInsertCheck(
						incomingTransaction, time))
		{
			return true;
		}
	...
	case OPEN_PAGE_AGGRESSIVE:
		...
		if (incomingTransaction->isRefresh())
		{
			...
			// therefore, we need to make sure that the refresh commands happen when all banks
			// are available
			for (vector<Bank>::const_iterator i = currentRank.bank.begin(); i
					!= currentRank.bank.end(); ++i)
			{
		...
		else if (!destinationBank.isFull())
		{
			...
			if ((systemConfig.getRowBufferManagementPolicy()
					== OPEN_PAGE_AGGRESSIVE)
					&& (destinationBank.openPageAggressiveInsertCheck(
							incomingTransaction, time)))
			{
				return true;
			}
			else
			{
				...
				if (((destinationBank.isEmpty()
						&& destinationBank.isActivated())
						|| (!destinationBank.isEmpty()
								&& !destinationBank.back()->isRefresh())))
				{
					if (destinationBank.freeCommandSlots() < 3)
					{
						return false;
					}
					assert(!destinationBank.back()
							|| destinationBank.back()->getCommandType()
									!= Command::REFRESH_ALL);
					assert(!destinationBank.back()
							|| destinationBank.back()->getCommandType()
									!= Command::PRECHARGE);
...
bool Channel::transaction2commands(Transaction *incomingTransaction)
{
	...
	assert(incomingTransaction->getAddress().getChannel() == channelID
			|| incomingTransaction->isRefresh());
	assert(incomingTransaction->getAddress().getChannel()
			< systemConfig.getChannelCount());
	assert(incomingTransaction->getAddress().getRank()
			< (systemConfig.getRankCount() * systemConfig.getDimmCount()));

	vector<Rank>::iterator destinationRank = rank.begin()
			+ incomingTransaction->getAddress().getRank();

	vector<Bank>::iterator destinationBank = destinationRank->bank.begin()
			+ incomingTransaction->getAddress().getBank();

	assert(incomingTransaction->getAddress().getBank()
			< systemConfig.getBankCount());
	assert(incomingTransaction->getAddress().getRow()
			< systemConfig.getRowCount());
	assert(incomingTransaction->getAddress().getColumn()
			< systemConfig.getColumnCount());
	...
	case CLOSE_PAGE_AGGRESSIVE:
		...
		if (incomingTransaction->isRefresh())
		{
			...
			Command *refreshCommand = new Command(incomingTransaction, time,
					systemConfig.isAutoPrecharge(),
					timingSpecification.tBurst());

			for (vector<Bank>::iterator currentBank =
					destinationRank->bank.begin(); currentBank
					!= destinationRank->bank.end(); ++currentBank)
			{
#ifndef NDEBUG
		...
		// or three commands if the CAS+Precharge command is not available
				...
				== CLOSE_PAGE_AGGRESSIVE && destinationBank->aggressiveInsert(
				incomingTransaction, time))
		{
		...
		// or three commands if the CAS+Precharge command is not available
				...
				|| (!systemConfig.isAutoPrecharge()
						&& (destinationBank->freeCommandSlots() < 3)))
		{
			return false;
		}
		...
		else
		{
			...
			bool result =
#endif
					destinationBank->push(new Command(incomingTransaction,
							time, false, timingSpecification.tBurst(),
							Command::ACTIVATE));
			assert(result);
			assert(destinationBank->back()->getAddress()
					== incomingTransaction->getAddress());
...
#endif
					destinationBank->push(new Command(incomingTransaction,
							time, systemConfig.isAutoPrecharge(),
							timingSpecification.tBurst()));
			assert(result);
			assert(destinationBank->back()->getAddress()
					== incomingTransaction->getAddress());
	...
	case OPEN_PAGE_AGGRESSIVE:
		...
		if (incomingTransaction->isRefresh())
		{
			...
			for (vector<Bank>::iterator i = destinationRank->bank.begin(); i
					!= bankEnd; ++i)
			{
	...
	case OPEN_PAGE:
		...
		if (incomingTransaction->isRefresh())
		{
			...
			for (vector<Bank>::const_iterator i = destinationRank->bank.begin(); i
					!= bankEnd; ++i)
			{
				if (i->back())
				...
				if (i->isFull() || (i->freeCommandSlots() < 2
						&& !i->back()->isPrecharge()))
					return false;
			}
			// then add the command to all queues
			Command *refreshCommand = new Command(incomingTransaction, time,
					false, timingSpecification.tBurst());
					...
					!= bankEnd; ++i)
			{
				...
				else if (i->isEmpty() && i->isActivated())
				{
					...
					bool result =
#endif // NDEBUG
							i->push(new Command(incomingTransaction, tempAddr,
									time, false, timingSpecification.tBurst(),
									Command::PRECHARGE));
					assert(result);
			...
			for (vector<Bank>::const_iterator
					i =
							rank[incomingTransaction->getAddress().getRank()].bank.begin(); i
					!= rank[incomingTransaction->getAddress().getRank()].bank.end(); ++i)
			{
		...
		else if (!destinationBank->isFull())
		{
			...
			else
			{
				...
				{
					assert(!destinationBank->back()
							|| !destinationBank->back()->isPrecharge());
					...
					else
					{
						...
						bool result =
#endif // NDEBUG
								destinationBank->push(new Command(
										incomingTransaction, time, false,
										timingSpecification.tBurst(),
										Command::PRECHARGE));
						assert(result);
					}
...
Command *Channel::getNextCommand(const Command *useThisCommand)
{
	...
	if (useThisCommand)
	{
		vector<Rank>::iterator currentRank = rank.begin()
				+ useThisCommand->getAddress().getRank();

		if (rank[useThisCommand->getAddress().getRank()].bank[useThisCommand->getAddress().getBank()].front()
				!= useThisCommand)
			assert(
					rank[useThisCommand->getAddress().getRank()].bank[useThisCommand->getAddress().getBank()].front()
							== useThisCommand);
		...
		{
			assert(
					(useThisCommand->isPrecharge()
							&& systemConfig.getRowBufferManagementPolicy()
									== OPEN_PAGE)
							|| (currentRank->bank[useThisCommand->getAddress().getBank()].front()
									== useThisCommand));

			vector<Bank>::iterator currentBank = currentRank->bank.begin()
					+ useThisCommand->getAddress().getBank();
			...
			if ((systemConfig.getRowBufferManagementPolicy()
					== OPEN_PAGE_AGGRESSIVE) && useThisCommand->isReadOrWrite()
					&& !currentBank->isEmpty()
...
const Command *Channel::readNextCommand() const
{
	...
	case FIRST_AVAILABLE_AGE:
	{
		...
		for (vector<Rank>::const_iterator currentRank = rank.begin(); currentRank
				!= rankEnd; ++currentRank)
		{
			...
			for (vector<Bank>::const_iterator currentBank =
					currentRank->bank.begin(); currentBank != bankEnd; ++currentBank)
			{
				if (!currentBank->isEmpty())
				{
					...
					assert(
							!challengerCommand
									|| challengerCommand->isRefresh()
									|| rank[challengerCommand->getAddress().getRank()].bank[challengerCommand->getAddress().getBank()].front()
											== challengerCommand);

					// see if it is a refresh command
					if (isRefreshCommand && challengerCommand->isRefresh()
							&& currentRank->refreshAllReady())
					{
						tick challengerExecuteTime = earliestExecuteTime(
								challengerCommand);
#ifndef NDEBUG
						...
						{
							cerr << "cet " << challengerExecuteTime
									<< ", mingap " << time + minGap << endl;
							cerr << challengerCommand << endl;
							assert(time + minGap == challengerExecuteTime);
						}
#endif
						if (challengerExecuteTime < candidateExecuteTime
								|| (candidateExecuteTime
										== challengerExecuteTime
										&& challengerCommand->getEnqueueTime()
												< candidateCommand->getEnqueueTime()))
						{
					...
					else if (!challengerCommand->isRefresh())
					{
						...
						if (time + minGap != challengerExecuteTime)
						{
							cerr << time << " " << minGap << " "
									<< challengerExecuteTime << " "
									<< challengerCommand->getCommandType()
									<< " " << challengerCommand->getAddress()
									<< endl;

							if (challengerCommand && (time + minGap
									!= challengerExecuteTime))
							{
								minGap = minProtocolGap(challengerCommand);
								assert(challengerCommand && (time + minGap
										== challengerExecuteTime));
							}
						}
#endif
		...
		{
			assert(
					candidateCommand->isRefresh()
							|| rank[candidateCommand->getAddress().getRank()].bank[candidateCommand->getAddress().getBank()].front()
									== candidateCommand);
				...
				if (!currentBank->isEmpty())
				{
							...
							&& currentRank->refreshAllReady())
					{
						...
						if ((challengerExecuteTime < candidateExecuteTime)
								|| (candidateExecuteTime
										== challengerExecuteTime
										&& ((challengerCommand->isRead()
												&& candidateCommand->isWrite())
												|| (challengerCommand->isRead()
														&& candidateCommand->isRead()
														&& challengerCommand->getEnqueueTime()
																< candidateCommand->getEnqueueTime())))
								|| (time - challengerCommand->getEnqueueTime()
										> 12
												* systemConfig.getSeniorityAgeLimit()))
						{
							candidateCommand = challengerCommand;
							...
							&& currentRank->refreshAllReady())
					{
						...
						if ((challengerExecuteTime < candidateExecuteTime)
								|| (candidateExecuteTime
										== challengerExecuteTime
										&& currentBank->size()
												> rank[candidateCommand->getAddress().getRank()].bank[candidateCommand->getAddress().getBank()].size())
								|| (time - challengerCommand->getEnqueueTime()
										> systemConfig.getSeniorityAgeLimit()))
						{
							candidateCommand = challengerCommand;
...
#ifndef NDEBUG
					{
						const tick executeTime = earliestExecuteTime(
								challengerCommand);
						if (executeTime != time + minProtocolGap(
								challengerCommand))
						{
							assert(executeTime == time + minProtocolGap(
									challengerCommand));
						}
					}
#endif
	...
	case RANK_ROUND_ROBIN:
	{
		...
		const Command::CommandType lastCommandType =
				lastCommand ? (lastCommand->getCommandType()) : Command::READ;
		...
		case Command::ACTIVATE:
		{
			...
			const Command *nextCommand = ((rank.begin()
					+ lastCommand->getAddress().getRank())->bank.begin()
					+ lastCommand->getAddress().getBank())->front();
			...
			{
				cerr
						<< "error: Found a row activate not followed by a column command."
						<< endl;
				exit(-2);
			}
		}
		...
		unsigned currentBankOffset =
				lastCommand ? lastCommand->getAddress().getBank() : 0;

		vector<Rank>::const_iterator currentRank = rank.begin()
				+ (lastCommand ? lastCommand->getAddress().getRank() : 0) + 1;
		if (currentRank == rank.end())
		{
			currentBankOffset = (currentBankOffset + 1)
					% systemConfig.getBankCount();
		...
		while (true)
		{
			const Command *potentialCommand = (currentRank->bank.begin()
					+ currentBankOffset)->front();
			...
			if (potentialCommand && !potentialCommand->isRefresh())
			{
				...
				if (systemConfig.isReadWriteGrouping())
				{
					if (potentialCommand->isActivate())
						assert(
								(currentRank->bank.begin() + currentBankOffset)->read(
										1));

					const Command
							*secondCommand =
									potentialCommand->isActivate() ? (currentRank->bank.begin()
											+ currentBankOffset)->read(1)
											: potentialCommand;
					...
					{
						assert(
								(currentRank->bank.begin() + currentBankOffset)->front()
										== potentialCommand);
			...
			if (currentBankOffset == startingBankOffset && currentRank
					== startingRank)
			{
				if (systemConfig.isReadWriteGrouping())
				{
					...
					{
#ifndef NDEBUG
						for (vector<Rank>::const_iterator i = rank.begin(); i
								!= rankEnd; ++i)
						{
							for (vector<Bank>::const_iterator j =
									i->bank.begin(); j != i->bank.end(); ++j)
							{
								assert(j->isEmpty());
							}
						}
	...
	case BANK_ROUND_ROBIN:
	{
		...
		case Command::ACTIVATE:
		{
			...
			{
				cerr
						<< "error: row activate command not followed by a column command."
						<< endl;
				exit(-2);
			}
		}
		...
		vector<Rank>::const_iterator currentRank = rank.begin()
				+ (lastCommand ? lastCommand->getAddress().getRank() : 0);
		vector<Bank>::const_iterator currentBank = currentRank->bank.begin()
				+ (lastCommand ? lastCommand->getAddress().getBank() : 0) + 1;
		...
		while (true)
		{
			...
			if (potentialCommand && !potentialCommand->isRefresh())
			{
				if (systemConfig.isReadWriteGrouping())
				{
					...
					const Command
							*secondCommand =
									(potentialCommand->isActivate()
											&& currentBank->read(1)) ? currentBank->read(
											1)
											: potentialCommand;
					assert(secondCommand->isReadOrWrite()
							|| secondCommand->isBasicPrecharge());
					...
					if ((secondCommand->isRead() && readSweep)
							...
							|| secondCommand->isBasicPrecharge())
					{
						assert(currentBank->front()->getAddress().getRank()
								== currentRank->getRankId());
	...
	case COMMAND_PAIR_RANK_HOPPING:
	{
		...
		while (true)
		{
			const Command *potentialCommand =
					((rank.begin() + memLocation.first)->bank.begin()
							+ memLocation.second)->front();
	...
	default:
	{
		cerr << "This configuration and algorithm combination is not supported"
				<< endl;
		exit(-2);
	}
		break;
...
{
	const unsigned rankCount = systemConfig.getDimmCount()
			* systemConfig.getRankCount();
	...
	cprhSequence[0]
			= pair<pair<unsigned, unsigned> , pair<unsigned, unsigned> > (pair<
					unsigned, unsigned> (0, bankCount / 2), pair<unsigned,
					unsigned> (0, 0));
	...
	for (unsigned i = 1; i < rankCount * bankCount; i++)
	{
		...
		lastCasRank = (lastCasBank == 0) ? (lastCasRank + 1) % rankCount
				: lastCasRank;
		...
		lastRasRank = (lastCasBank == 0) ? lastRasRank : (lastRasRank + 1)
				% rankCount;
		lastRasBank = (lastCasBank == 0) ? bankCount / 2 : lastRasBank
				>= bankCount / 2 ? (lastRasBank + bankCount / 2) % bankCount
				: lastRasBank + bankCount / 2 + 1;
...
void Channel::setLastCprhLocation(unsigned rank, unsigned bank, bool isActivate)
{
	...
	{
		pair < pair<unsigned, unsigned> , pair<unsigned, unsigned> > memValue
				= cprhSequence[location];
		unsigned currentRank = isActivate ? memValue.first.first
				: memValue.second.first;
		unsigned currentBank = isActivate ? memValue.first.second
				: memValue.second.second;
...
#endif
	pair < pair<unsigned, unsigned> , pair<unsigned, unsigned> > loc
			= cprhSequence[location / 2];
...
{
	DPRINTF(MemoryAccess, "C %s", *thisCommand);

	vector<Command *> commandsExecuted;

	vector<Rank>::iterator currentRank = rank.begin()
			+ thisCommand->getAddress().getRank();

	vector<Bank>::iterator currentBank = currentRank->bank.begin()
			+ thisCommand->getAddress().getBank();
	...
	{
		setLastCprhLocation(thisCommand->getAddress().getRank(),
				thisCommand->getAddress().getBank(), thisCommand->isActivate());
	}
	...
	case Command::ACTIVATE:
	{
		...
		thisCommand->setCompletionTime(thisCommand->getStartTime()
				+ timingSpecification.tCMD() + timingSpecification.tRAS());
	}
		break;
	...
	case Command::READ:
	{
		...
		thisCommand->setCompletionTime(max(currentBank->getLastRasTime()
				+ timingSpecification.tRCD() + timingSpecification.tCAS()
				+ timingSpecification.tBurst(), time
				+ timingSpecification.tAL() + timingSpecification.tCAS()
				+ timingSpecification.tBurst()));
		thisCommand->getHost()->setCompletionTime(
				thisCommand->getCompletionTime());
	...
	case Command::WRITE:
		...
		assert(wasActivated);
		{
			thisCommand->setCompletionTime(time + timingSpecification.tCMD()
					+ timingSpecification.tCWD() + timingSpecification.tBurst());
	...
	case Command::PRECHARGE:
		...
		thisCommand->setCompletionTime(thisCommand->getStartTime()
				+ timingSpecification.tCMD() + timingSpecification.tRP());
...
tick Channel::minProtocolGap(const Command *currentCommand) const
{
	...
	const Bank &currentBank =
			currentRank.bank[currentCommand->getAddress().getBank()];
	...
	case Command::ACTIVATE:
	{
		...
		tick tRCGap = (tick)(currentBank.getLastRasTime() - time)
				+ timingSpecification.tRC();

		// respect tRRD and tRC of all other banks of same rank
		tick tRRDGap = (tick)(currentRank.lastActivateTimes.front() - time)
				+ timingSpecification.tRRD();

		// respect tRP of same bank
		tick tRPGap = (tick)(currentBank.getLastPrechargeTime() - time)
				+ timingSpecification.tRP();

		// respect the t_faw value for DDR2 and beyond
		tick tFAWGap = (tick)(currentRank.lastActivateTimes.back() - time)
				+ timingSpecification.tFAW();

		// respect tRFC
		tick tRFCGap = (tick)(currentRank.getLastRefreshTime() - time)
				+ timingSpecification.tRFC();
	...
	case Command::READ:
	{
		...
		tick tRCDGap = ((currentBank.getLastRasTime() - time)
				+ timingSpecification.tRCD() - timingSpecification.tAL());
		...
		for (vector<Rank>::const_iterator thisRank = rank.begin(); thisRank
				!= rank.end(); ++thisRank)
		{
		...
		tick t_cas_gap = ((currentRank.getLastCasTime() - time)
				+ timingSpecification.tBurst());
		...
		t_cas_gap = max((tick) t_cas_gap, ((currentRank.getLastCaswTime()
				- time) + timingSpecification.tCWD()
				+ timingSpecification.tBurst() + timingSpecification.tWTR()));

		if (rank.size() > 1)
		{
			//respect most recent cas of different rank
			t_cas_gap = max((tick) t_cas_gap, (otherRankLastCASTime - time)
					+ otherRankLastCASLength + timingSpecification.tRTRS());
			//respect timing of READ follow WRITE, different ranks
			t_cas_gap = max((tick) t_cas_gap, (otherRankLastCASWTime - time)
					+ timingSpecification.tCWD() + otherRankLastCASWLength
					+ timingSpecification.tRTRS() - timingSpecification.tCAS());
		}
	...
	{
		//respect last ras of same rank
		tick t_ras_gap = ((currentBank.getLastRasTime() - time)
				+ timingSpecification.tRCD() - timingSpecification.tAL());
		...
		tick t_cas_gap = (tick)(currentRank.getLastCasTime() - time)
				+ timingSpecification.tCAS() + timingSpecification.tBurst()
				+ timingSpecification.tRTRS() - timingSpecification.tCWD();

		// respect last cas to different ranks
		t_cas_gap = max((tick) t_cas_gap, (otherRankLastCASTime - time)
				+ timingSpecification.tCAS() + otherRankLastCASLength
				+ timingSpecification.tRTRS() - timingSpecification.tCWD());

		// respect last cas write to same rank
		// DW 3/9/2006 replace the line after next with the next line
		// t_cas_gap = max(t_cas_gap,(int)(this_r.last_casw_time + casw_length - now));
		t_cas_gap = max((tick) t_cas_gap,
				(currentRank.getLastCaswTime() - time)
						+ currentRank.getLastCaswLength());

		// respect last cas write to different ranks
		t_cas_gap = max((tick) t_cas_gap, (otherRankLastCASWTime - time)
				+ otherRankLastCASWLength + timingSpecification.tOST());
	...
	case Command::PRECHARGE:
	{
		...
		tick t_ras_gap = (currentBank.getLastRasTime() - time)
				+ timingSpecification.tRAS();

		// respect t_cas of same bank
		tick t_cas_gap = ((tick)(currentBank.getLastCasTime() - time)
				+ timingSpecification.tAL() + timingSpecification.tCAS()
				+ timingSpecification.tBurst() + max(0,
				timingSpecification.tRTP() - timingSpecification.tCMD()));

		// respect t_casw of same bank
		t_cas_gap = max((tick) t_cas_gap, ((currentBank.getLastCaswTime()
				- time) + timingSpecification.tAL()
				+ timingSpecification.tCWD() + timingSpecification.tBurst()
				+ timingSpecification.tWR()));
	...
	case Command::REFRESH_ALL:
		...
		min_gap
				= max((currentRank.getLastRefreshTime()
						+ timingSpecification.tRFC()),
						(currentRank.getLastPrechargeTime()
								+ timingSpecification.tRP())) - time;
		break;
	...
	return max(min_gap, lastCommandIssueTime + (tick)(
			timingSpecification.tCMD()) - time);
}
...
{
	const vector<Rank>::const_iterator currentRank = rank.begin()
			+ currentCommand->getAddress().getRank();

	const vector<Bank>::const_iterator currentBank = currentRank->bank.begin()
	...
	case Command::ACTIVATE:
	{
		...
		// respect the row cycle time limitation
		tick tRCLimit = currentBank->getLastRasTime()
				+ timingSpecification.tRC();

		// respect the row-to-row activation delay for different banks within a rank
		tick tRRDLimit = currentRank->lastActivateTimes.front()
				+ timingSpecification.tRRD();

		// respect tRP of same bank
		tick tRPLimit = currentBank->getLastPrechargeTime()
				+ timingSpecification.tRP();

		// respect the t_faw value for DDR2 and beyond, look at the fourth activate ago
		tick tFAWLimit = currentRank->lastActivateTimes.back()
				+ timingSpecification.tFAW();

		// respect tRFC, refresh cycle time
		tick tRFCLimit = currentRank->getLastRefreshTime()
				+ timingSpecification.tRFC();

		nextTime = max(max(max(tRFCLimit, tRCLimit), tRPLimit), max(tRRDLimit,
				tFAWLimit));

		assert(nextTime >= currentBank->getLastPrechargeTime()
				+ timingSpecification.tRP());
	...
	{
		//respect last RAS of same rank
		tick tRCDLimit = currentBank->getLastRasTime()
				+ (timingSpecification.tRCD() - timingSpecification.tAL());
		...
		// DW 3/9/2006 replace the line after next with the next line
		//t_cas_gap = max(0,(int)(this_r.last_cas_time + cas_length - now));
		tick tCASLimit = currentRank->getLastCasTime()
				+ timingSpecification.tBurst();

		// respect last CASW of same rank
		// DW 3/9/2006 replace the line after next with the next line
		//t_cas_gap = max(t_cas_gap,(int)(this_r.last_casw_time + timing_specification.t_cwd + casw_length + timing_specification.t_wtr - now));
		tCASLimit = max(tCASLimit, currentRank->getLastCaswTime()
				+ timingSpecification.tCWD() + timingSpecification.tBurst()
				+ timingSpecification.tWTR());

		//if (rank->size() > 1)
		{
			//respect most recent CAS of different rank
			tCASLimit = max(tCASLimit, currentRank->getOtherLastCasTime()
					+ currentRank->getOtherLastCasLength()
					+ timingSpecification.tRTRS());
			//respect timing of READ follow WRITE, different ranks
			tCASLimit = max(tCASLimit, currentRank->getOtherLastCaswTime()
					+ timingSpecification.tCWD()
					+ currentRank->getOtherLastCaswLength()
					+ timingSpecification.tRTRS() - timingSpecification.tCAS());
		}
	...
	{
		//respect last RAS of same rank
		tick tRASLimit = currentBank->getLastRasTime()
				+ timingSpecification.tRCD() - timingSpecification.tAL();
	...
	case Command::PRECHARGE:
	{
		...
		tick tCASLimit = currentBank->getLastCasTime()
				+ timingSpecification.tAL() + timingSpecification.tCAS()
				+ timingSpecification.tBurst() + max(0,
				timingSpecification.tRTP() - timingSpecification.tCMD());
		...
		// tAL is accounted for by measuring the execution time internal to the DRAM
		tCASLimit = max(tCASLimit, currentBank->getLastCaswTime()
				+ timingSpecification.tAL() + timingSpecification.tCWD()
				+ timingSpecification.tBurst() + timingSpecification.tWR());
	...
	tick actualNext = max(nextTime, lastCommandIssueTime
			+ timingSpecification.tCMD());
	tick predictedNext = max(
			currentRank->next(currentCommand->getCommandType()), max(
					currentBank->next(currentCommand->getCommandType()),
					lastCommandIssueTime + timingSpecification.tCMD()));
	if (actualNext != predictedNext)
		assert(actualNext == predictedNext);
#endif

	return max(currentRank->next(currentCommand->getCommandType()), max(
			currentBank->next(currentCommand->getCommandType()),
			lastCommandIssueTime + timingSpecification.tCMD()));
}
...
tick Channel::earliestExecuteTimeLog(const Command *currentCommand) const
{
	...
	static unsigned tRCDcount, tCAScount, tCASWcount, tCASotherCount,
			tCASWotherCount;
	...
	case Command::READ:
	{
		...
		// DW 3/9/2006 replace the line after next with the next line
		//t_cas_gap = max(t_cas_gap,(int)(this_r.last_casw_time + timing_specification.t_cwd + casw_length + timing_specification.t_wtr - now));
		tick tCASWLimit = currentRank.getLastCaswTime()
				+ timingSpecification.tCWD() + timingSpecification.tBurst()
				+ timingSpecification.tWTR();

		//respect most recent CAS of different rank
		tick tCASOtherLimit = currentRank.getOtherLastCasTime()
				+ currentRank.getOtherLastCasLength()
				+ timingSpecification.tRTRS();
		//respect timing of READ follow WRITE, different ranks
		tick tCASWOtherLimit = currentRank.getOtherLastCaswTime()
				+ timingSpecification.tCWD()
				+ currentRank.getOtherLastCaswLength()
				+ timingSpecification.tRTRS() - timingSpecification.tCAS();

		nextTime = max(max(tRCDLimit, tCASLimit), max(max(tCASWLimit,
				tCASOtherLimit), tCASWOtherLimit));
	...
	case Command::WRITE:
	{
		...
		// DW 3/9/2006 replace the line after next with the next line
		// t_cas_gap = max(0,(int)(this_r.last_cas_time + timing_specification.t_cas + cas_length + timing_specification.t_rtrs - timing_specification.t_cwd - now));
		tick tCASLimit = max(time, currentRank.getLastCasTime()
				+ timingSpecification.tCAS() + timingSpecification.tBurst()
				+ timingSpecification.tRTRS() - timingSpecification.tCWD());
		...
		nextTime = max(max(tRASLimit, tCASLimit), max(max(tCASOtherLimit,
				tCASWOtherLimit), tCASWLimit));
	...
	case Command::PRECHARGE:
	{
		...
		//tick tCASLimit = max(time,currentBank.getLastCasTime() + timingSpecification.tAL() + timingSpecification.tCAS() + timingSpecification.tBurst() + max(0,timingSpecification.tRTP() - timingSpecification.tCMD()));
		// tAL is accounted for by measuring the execution time internal to the DRAM
		tick tCASLimit = max(time, currentBank.getLastCasTime()
				+ timingSpecification.tCAS() + timingSpecification.tBurst()
				+ max(0, timingSpecification.tRTP()
						- timingSpecification.tCMD()));
	...
	case Command::REFRESH_ALL:
		// respect tRFC and tRP
		nextTime = max(currentRank.getLastRefreshTime()
				+ timingSpecification.tRFC(),
				currentRank.getLastPrechargeTime() + timingSpecification.tRP());
		break;
	...
	return max(nextTime, max(time, lastCommandIssueTime
			+ timingSpecification.tCMD()));
}
...
{
	return (time == rhs.time && lastCommandIssueTime
			== rhs.lastCommandIssueTime && timingSpecification
			== rhs.timingSpecification && transactionQueue
			== rhs.transactionQueue && ((lastCommand == NULL && rhs.lastCommand
			== NULL) || (*lastCommand == *(rhs.lastCommand))) && systemConfig
			== rhs.systemConfig && statistics == rhs.statistics && powerModel
			== rhs.powerModel && channelID == rhs.channelID && rank == rhs.rank
			&& refreshCounter == rhs.refreshCounter);
}
...
/// @author Joe Gross
//////////////////////////////////////////////////////////////////////////
std::ostream& DRAMsimII::operator<<(std::ostream& os,
		const DRAMsimII::Channel& r)
{
