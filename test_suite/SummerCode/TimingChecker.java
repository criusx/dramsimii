...
public class TimingChecker {
	...
	public static void main(String[] args) {
		...
		HashMap<String,Integer> timingParameters = new HashMap<String, Integer>();
		...
		try{
			...
			NodeList timingParams = xmlDoc.getElementsByTagName("timing").item(0).getChildNodes();
			
			for(int i = 0; i< timingParams.getLength(); i++){
				if(timingParams.item(i).getNodeType()!=Node.COMMENT_NODE&&!timingParams.item(i).getTextContent().isEmpty()&&
						timingParams.item(i).getNodeName().startsWith("t") && !timingParams.item(i).getNodeName().startsWith("transaction")){
					System.out.println(timingParams.item(i).getNodeName() + " : "  + timingParams.item(i).getTextContent());
					timingParameters.put(timingParams.item(i).getNodeName(), Integer.valueOf(timingParams.item(i).getTextContent()));
				}
			}
			
			//check for timing parameters that don't make sense
			
			if(timingParameters.get("tRAS")+timingParameters.get("tRP")< timingParameters.get("tRC")){
				System.out.println("tRAS + tRP < tRC");
			}
			if(timingParameters.get("tRCD")+timingParameters.get("tCWD")+timingParameters.get("tBurst")+timingParameters.get("tWR")<timingParameters.get("tRAS")){
			...
			if(timingParameters.get("tRCD") + timingParameters.get("tBurst") + timingParameters.get("tRTP") - timingParameters.get("tCCD") < timingParameters.get("tRAS")){
