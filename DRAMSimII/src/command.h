...
namespace DRAMSimII
{
	...
	private:
		...
		command_type_t commandType;	// which command is this?
		tick_t startTime;				// when did the command start?
		tick_t enqueueTime;			// when did it make it into the per bank command queue?
		tick_t completionTime;
		addresses addr;
	...
	public:
		...
		//bool isRefresh() const { return refresh; }
