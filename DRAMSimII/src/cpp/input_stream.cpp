...
double input_stream_c::gammaln(double xx)
{
  double x, y, tmp, ser;
...
enum input_status_t input_stream_c::get_next_bus_event(bus_event &this_e)
{
  ...
  if(type == K6_TRACE){
    while((bursting == true) && (EOF_reached == false)){
      ...
      if((this_e.attributes != control) || (((this_e.address ^ address) & 0xFFFFFFE0) != 0) ||
...
enum input_type_t input_stream_c::input_token(string input)
{
  if (input == "k6" || input == "K6")
    return K6_TRACE;
  else if (input == "mase" || input == "MASE")
    return MASE_TRACE;
