...
public:
  ...
  addresses	addr;
  const transaction *host_t;	/* backward pointer to the original transaction */
...
public:
  ...
  enum transaction_type_t	set_read_write_type(int, int);
};
...
class input_stream_c {
  //protected:
private:
  enum input_type_t input_token(string);
...
class dram_system
 // public dram_system_configuration,
  //public dram_timing_specification,
  //public simulation_parameters,
  //public dram_statistics,
  //public dram_algorithm
{
private:
  ...
  enum input_status_t transaction2commands(const transaction*);
  int min_protocol_gap(int,const command *);
  int find_oldest_channel();
  void execute_command(command *, int );
  void update_system_time();
  int convert_address(addresses &);
  enum input_status_t get_next_input_transaction(transaction*&);
  transaction *get_next_random_request();
