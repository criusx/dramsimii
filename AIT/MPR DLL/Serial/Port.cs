...
//		private bool	isOpen		= false;
		...
		public byte[] Output
		{
			set
			{
				...
				// Send the whole array, irregardless of sthreshold
			...
			try
			{
				...
				while(hPort != (IntPtr)CommAPI.INVALID_HANDLE_VALUE)
				{
					...
					if(!m_CommAPI.WaitCommEvent(hPort, ref eventFlags))
					{
						...
						switch (e) 
						{
							case (int)APIErrors.ERROR_IO_PENDING:
								// IO pending so just wait and try again
								rxevent.WaitOne();
								Thread.Sleep(0);
								continue;

							case (int)APIErrors.ERROR_INVALID_HANDLE:
								...
								if(eventResult == (int)APIConstants.WAIT_OBJECT_0)
								{
									...
									{
										Close(); // Kill the Port, to release the windows handle to it
										// The card was ejected, so alert the user
										if (IsOpenChanged != null)
											IsOpenChanged(false);
									}
							...
							case (int)APIErrors.ERROR_OPERATION_ABORTED:
								// 995 means an exit was requested (thread or COM Port killed)
								Close(); // Kill the Port, to release the windows handle to it
								if (IsOpenChanged != null)
									IsOpenChanged(false);
								return; // Abort thread!
							...
							default:
								throw new CommPortException(string.Format("Wait Failed: {0}", (int)e));
						}
					}
